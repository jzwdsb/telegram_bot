...
impl DynamoDbStorage {
    ...
    pub async fn list_all_preferences(&self) -> Result<Vec<UserPreferences>, StorageError> {
        ...
        if let Some(items) = result.items {
            for item in items {
                if let Some(chat_id) = item.get("chat_id").and_then(|v| v.as_s().ok())
                    && let Some(model) = item.get("ai_model").and_then(|v| v.as_s().ok())
                    && let Some(updated_at) = item.get("updated_at").and_then(|v| v.as_s().ok())
                {
                    let expires_at = item
                        .get("expires_at")
